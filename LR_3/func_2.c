...
__IO uint16_t ADCConvertedValue[4]; //Для 4-х каналов
...
void Func_2(void)
{
    ...
    ADC_Init(ADC1, &ADC_InitStructure); //Инициализация ADC
     ...
     /* Конфигурмрование контроллера прямого доступа к памяти DMA1 channel1------*/
   ...
   Int prevVal = ADCConvertedValue[0];
       while (1) {
    	   // Наклон вверх/вниз
If ((ADCConvertedValue[0] – prevVal) < 0) {
	change_light(GPIO_Pin_9, Bit_SET);
} else if (((ADCConvertedValue[0] – prevVal) > 0) {
	change_light(GPIO_Pin_7, Bit_SET);

} else {}
    	   if(ADCConvertedValue[0]>3800) { // Вниз
    		   change_light(GPIO_Pin_9, Bit_SET);
    	   }
    	   if(ADCConvertedValue[0]<100) { // Вверх
    		   change_light(GPIO_Pin_7, Bit_SET);
    	   }
    	   if(ADCConvertedValue[0]>1200 & ADCConvertedValue[0]<2400) {
    		   change_light(GPIO_Pin_9, Bit_RESET);
    		   change_light(GPIO_Pin_7, Bit_RESET);

    	   }
    	   // Наклон влево/вправо
    	   if(ADCConvertedValue[1]>3800) { // Влево
    		   change_light(GPIO_Pin_6, Bit_SET);
    	   }
    	   if(ADCConvertedValue[1]<100) { // Вправо
    		   change_light(GPIO_Pin_8, Bit_SET);
    	   }
    	   if(ADCConvertedValue[1]>1000 & ADCConvertedValue[1]<2000) {
    	   ...
    	   if(ADCConvertedValue[2]<800) { // Нажатие
    		   change_light(GPIO_Pin_6, Bit_SET);
    		   change_light(GPIO_Pin_7, Bit_SET);
    		   change_light(GPIO_Pin_8, Bit_SET);
    		   change_light(GPIO_Pin_9, Bit_SET);
    	   ...
    	   if(ADCConvertedValue[2]>1000) { // Отжатие
    	       change_light(GPIO_Pin_6, Bit_RESET);
    	       change_light(GPIO_Pin_7, Bit_RESET);
    	       change_light(GPIO_Pin_8, Bit_RESET);
    	       change_light(GPIO_Pin_9, Bit_RESET);
